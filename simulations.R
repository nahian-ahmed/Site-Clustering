...
# Fixed Skew Pattern: Centers (Modified Intensity)
# Sampling Strategies: Uniform, Positive (Hotspot), Negative (Coldspot)
# -----------------------------------------------------------------
...
n_sampling_clusters <- 5   # Number of hotspots/coldspots
sampling_sigma <- 20       # Radius of the sampling hotspot (spatial spread)

# --- Spatial Autocorrelation (SAC) Settings ---
sac_levels <- c("Low", "Medium", "High") 
# UPDATED: Wider range to make High SAC distinct
sac_sigmas <- c(Low = 0, Medium = 10, High = 40) 
...
n_centers <- 1
# UPDATED: Reduced scale so SAC noise is visible
centers_scale <- 1.5 
decay_scale <- 30^2
...
cat(sprintf("TOTAL ITERATIONS: %d\n\n", 
            length(sac_levels) * n_sims * length(sampling_strategies) * length(M_values_to_test)))
...
total_rows <- length(sac_levels) * n_sims * length(sampling_strategies) * length(M_values_to_test)
results_list <- vector("list", total_rows)
results_counter <- 1
...
for (sac_level in sac_levels) {
  ...
  for (sim in 1:n_sims) {
    ...
    ##########
    # 7. Simulate Full Observation Data (y)
    ##########
    ...
    # --- PREPARE SPATIAL INFO FOR SAMPLING ---
    # Get Site Centroids for distance calculations
    site_xs <- ((0:(full_M-1) %% full_n_sites_x) + 0.5) * site_dim
    site_ys <- ((0:(full_M-1) %/% full_n_sites_x) + 0.5) * site_dim
    site_coords <- cbind(x=site_xs, y=site_ys)
    
    # Get Site-Level Covariate (Average or Sum depending on W) for sampling probability
    site_cov_vals <- as.vector(full_w %*% full_cellCovs$cell_cov1)
    
    # Normalize Covariates to [0,1] for Probability Selection
    min_c <- min(site_cov_vals)
    max_c <- max(site_cov_vals)
    cov_norm <- (site_cov_vals - min_c) / (max_c - min_c + 1e-6)
    ...
    for (sampling_strat in sampling_strategies) {
      ...
      # --- Generate Sampling Weights based on Clusters ---
      if (sampling_strat == "Uniform") {
        curr_weights <- NULL
      } else {
        # 1. Determine Probability of being a Center
        if (sampling_strat == "Positive") {
          # High Cov = High Prob
          center_probs <- cov_norm
        } else { # Negative
          # Low Cov = High Prob
          center_probs <- 1 - cov_norm
        }
        
        # 2. Pick Clusters (Hotspots)
        center_ids <- sample(1:full_M, n_sampling_clusters, prob = center_probs, replace = FALSE)
        center_coords <- site_coords[center_ids, , drop=FALSE]
        
        # 3. Calculate Distance from every site to the NEAREST cluster center
        min_dists <- numeric(full_M)
        for(s in 1:full_M) {
          dists <- sqrt((site_coords[s,1] - center_coords[,1])^2 + 
                          (site_coords[s,2] - center_coords[,2])^2)
          min_dists[s] <- min(dists)
        }
        
        # 4. Assign Weights using Gaussian Decay
        curr_weights <- exp(-min_dists^2 / (2 * sampling_sigma^2))
      }
      ...
      for (M_i in M_values_to_test) {
        ...
        ##########
        # 12. Plotting (Sim 1, Smallest M Only)
        ##########
        if (sim == 1 && M_i == M_values_to_test[1]) {
          # Only plotting Uniform for brevity, or add logic to plot all strats
          if(sampling_strat == "Uniform") {
            cell_df <- data.frame(
              x = full_cell_col,
              y = full_cell_row,
            ...
            p_cov <- ggplot(cell_df, aes(x=x, y=y, fill=covariate)) +
              ...
              labs(title=sprintf("Covariate (M=%d, %s)", M, sampling_strat), fill="Covariate") +
              tight_theme
            ...
            p_abund <- ggplot(cell_df, aes(x=x, y=y, fill=site_latent_abundance)) +
              ...
              geom_rect(data=site_boxes, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax),
                        color="red", fill=NA, linewidth=0.3, inherit.aes=FALSE) +
              labs(title=sprintf("Abundance (M=%d, %s)", M, sampling_strat), fill="Abundance") +
              tight_theme
            ...
            p_occ <- ggplot(cell_df, aes(x=x, y=y, fill=site_true_occupancy)) +
              ...
              geom_rect(data=site_boxes, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax),
                        color="red", fill=NA, linewidth=0.3, inherit.aes=FALSE) +
              labs(title=sprintf("Occupancy (M=%d, %s)", M, sampling_strat), fill="Occupancy") +
              tight_theme
    ...
    # Save Plots for this SAC level (Only saving Uniform plots for now to avoid clutter)
    if (sim == 1) {
...
if(nrow(strat_df) > 0) {
  ...
  # Create a Faceted Plot to show Sampling Strategies
  create_error_plot <- function(param_name, title) {
    ggplot(strat_df[strat_df$Parameter == param_name, ], 
      ...
      facet_wrap(~Sampling, ncol=1) + # Facet by Sampling Strategy
      scale_fill_manual(values = c("Low" = "yellow", "Medium" = "orange", "High" = "red")) +
      labs(title = title, x = "M (Sites)", y = "Error", fill = "SAC") +
  ...
  ggsave(file.path(output_dir, fname), plot = combined_error_plot, dpi = 300, width = 12, height = 12)
  cat(sprintf("Saved %s\n", fname))
}
