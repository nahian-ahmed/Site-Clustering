...
#'   'occupied_prob', 'occupied', 'detection', and 'species_observed'.
...
summarize_datasets <- function(train_data, test_data) {
  ...
  # 'occupied_prob' (psi_i) and 'occupied' (Z_i) are constant for all
  # checklists at a given site.
  train_sites <- dplyr::distinct(train_data, site, .keep_all = TRUE)
  
  # 1. Mean site-level occupancy probability (psi_i)
  #    This is the metric from your data that corresponds to the
  #    site-level state, prior to the Bernoulli trial.
  train_mean_site_psi <- mean(train_sites$occupied_prob, na.rm = TRUE)
  ...
  # 5. Mean point-level occupancy probability (psi_j)
  test_point_mean_psi <- mean(test_data$occupied_prob, na.rm = TRUE)
  ...
  stats_row <- data.frame(
    train_mean_site_psi = train_mean_site_psi,
    ...
    test_point_mean_psi = test_point_mean_psi,
