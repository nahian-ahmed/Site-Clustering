...
# (Updated: Fixed rho = 0.5, Optimizes Kappa only)
###########################
...
bayesianOptimizedClustGeo <- function(
  ...
  # --- 3. FITNESS FUNCTION ---
  # MODIFIED: Accepts only kappa, rho is fixed
  clustGeo_fit <- function(kappa) { 
    
    tryCatch({
      # A. Generate Clusters
      # MODIFIED: Hardcoded alpha (rho) to 0.5
  ...
  # --- 4. RUN OPTIMIZATION ---
  # MODIFIED: Removed rho from bounds and grid
  ...
  # MODIFIED: Retrieve best kappa only
  best_kappa <- res$Best_Par["kappa"]
  
  # Rerun with fixed rho = 0.5
  ...
  return(list(
    Optimization = res,
    Final_Site_Map = train_locs[, c("locality_id", "site_id")]
  ))
}