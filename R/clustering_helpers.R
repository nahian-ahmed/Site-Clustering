...
.run_clustgeo_internal <- function(data, rho, percent, state_covs) {
  ...
  data_cgul$site.new <- NULL 
  data_cgul$site[is.na(data_cgul$site)] <- -1 
  
  return(data_cgul)
}


# 3. DISPATCHER FUNCTION (Removed cov_raster)
run_clustering_method <- function(method_name, og_data, state_covs, truth_df = NULL) {
  ...
  if (base_method == "kmSq") {
    ...
    rad_m <- as.integer(sqrt(area_kmSq) * 1000) 
  ...
  # --- B. Complex methods ---
    ...
    # Kept for compatibility, though usually called directly in species_clusters.R
  ...
  # --- C. Simple/Flag-based methods ---
    ...
    if (method_name == "one_UL" || method_name == "1-per-UL") {
      df_1_UL_t <- auk::filter_repeat_visits(
        og_data, min_obs = 1, max_obs = 1000000, annual_closure = TRUE,
        date_var = "formatted_date", site_vars = c("locality_id")
      )
      result_df <- df_1_UL_t %>% group_by(site) %>% filter(row_number() == 1) %>% ungroup() 
...
# 4. THE NEW, CLEAN getClusterings FUNCTION (Removed cov_raster)
