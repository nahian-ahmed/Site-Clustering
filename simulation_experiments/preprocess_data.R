...
data_path <- "results/simulation_experiments/raw/data/"
species_names = as.list(species_df$species)
...
for (spec_name in species_names){
    ...
    if (!dir.exists(paste0("results/simulation_experiments/raw/test_data_splits/", spec_name))) {
        dir.create(paste0("results/simulation_experiments/raw/test_data_splits/", spec_name))
    }  

    f.name_test <- paste0(spec_name, "_test.csv")
    ...
    for(exp_run in seq(1:runs)){
        ...
        write.csv(test.df, paste0("results/simulation_experiments/raw/test_data_splits/", spec_name, "/run=", exp_run,".csv"), row.names=FALSE)
    }
}