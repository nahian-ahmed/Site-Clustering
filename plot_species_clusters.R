...
# --- EXTRACTION: (Mean % Improvement) Values (Mean % Improvement) ---

# 1. Calculate the exact mean % improvment values for the SPECIES plot
mean_auc_species <- auc_by_species %>%
  group_by(method) %>%
  summarise(mean_percentage_difference = mean(mean_perc_diff, na.rm = TRUE)) %>%
  arrange(desc(mean_percentage_difference))

# 2. Calculate the exact mean % improvment values for the REPEATS plot
mean_auc_repeats <- auc_by_repeat %>%
  group_by(method) %>%
  summarise(mean_percentage_difference = mean(mean_perc_diff, na.rm = TRUE)) %>%
  arrange(desc(mean_percentage_difference))

# 3. Save these two-column summaries to CSV
write.csv(mean_auc_species, file.path(output_plot_dir, "mean_auc_perc_improvement_by_species.csv"), row.names = FALSE)
write.csv(mean_auc_repeats, file.path(output_plot_dir, "mean_auc_perc_improvement_by_repeats.csv"), row.names = FALSE)

cat("Mean % AUC improvment summary CSVs saved.\n")
# -------------------------------------------------------------
# -------------------------------------------------------------------------
...
for (sp in species_list) {
  ...
  for (i in seq_along(methods_for_maps)) {
    for (res in target_res) {
      ...
      scale_plots[[plot_idx]] <- ggplot() +
        ...
        labs(title = col_title, y = row_label) +
        theme(
          legend.position = "bottom", legend.text = element_text(size = 18), legend.title = element_text(size = 20, vjust = 1), 
          legend.key.width = unit(3, "cm"), legend.box.margin = margin(t = 20), plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
  ...
  ggsave(file.path(map_output_dir, paste0(sp, "_scales.png")), 
         plot = final_scales, width = 14, height = 32, dpi = 240, 
